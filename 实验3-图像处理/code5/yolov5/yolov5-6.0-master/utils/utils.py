    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                im = cv2.resize(cutout, (640, 480))  # BGR
                ...
                img_bin = get_BlueImg_bin(im)
                points = fixPosition(im, img_bin)
                ...
                if is_change_valid(point_set_0):
                    lic = transform(im, point_set_0, point_set_1)
                else:
                    lic = cv2.resize(cutout, (94, 24))

                # print(point_set_0)
                # plt.subplot(1, 3, 1)
                # plt.imshow(im[:, :, ::-1])
                # plt.subplot(1, 3, 2)
                # img_draw = cv2.drawContours(im.copy(), [new_box], -1, (0, 0, 255), 3)
                # plt.imshow(img_draw[:, :, ::-1])
                # plt.subplot(1, 3, 3)
                # plt.imshow(lic[:, :, ::-1])
                # plt.show()

                im = transform4torch(lic)
                ims.append(im)

            # rec
            # print(torch.Tensor(ims).shape)
            preds = model(torch.Tensor(np.array(ims)).to(d.device))  # classifier prediction
            # preds = model(torch.Tensor(ims))  # classifier prediction
            # preds = model(Variable(ims))
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        c2 = c1[0] + t_size[0] - 45, c1[1] - t_size[1] - 5
