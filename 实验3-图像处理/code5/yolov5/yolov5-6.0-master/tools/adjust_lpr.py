...
def main(path):
    ...
    for filename in os.listdir(path):
        ...
        # 图像预处理
        img, img_Gas, img_B, img_G, img_R, img_gray, img_HSV = imgProcess(img_path)
        ...
        if is_change_valid(point_set_0):
            ...
            lic = cv2.imread(img_path)
        lic = Image.fromarray(lic)
        lic = lic.resize((94, 24), Resampling.LANCZOS)
        lic = np.asarray(lic)
        cv2.imencode('.jpg', lic)[1].tofile(
            r"C:\Code\Licence-Recognition\datasets_LPRNet\adj_images\train\{}.jpg".format(result))
        num += 1
        if num % 2000 == 0:
            print("已完成" + str(num))
...
if __name__ == '__main__':
    path = r'C:\Code\Licence-Recognition\datasets_LPRNet\rec_images\train'
    main(path)
